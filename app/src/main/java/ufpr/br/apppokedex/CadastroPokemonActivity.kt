...
class CadastroPokemonActivity : AppCompatActivity() {
    ...
    private var pokemonId: Int? = null  // se for edição => terá valor
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // verificar se está em edição ou criação de um pokemon
        pokemonId = intent.getIntExtra("pokemon_id", -1).takeIf { it != -1 }
        if (pokemonId != null) {
            val nome = intent.getStringExtra("pokemon_nome")
            val tipo = intent.getStringExtra("pokemon_tipo")
            val habilidades = intent.getStringArrayListExtra("pokemon_habilidades")

            etNome.setText(nome)
            etTipo.setText(tipo)
            etHab1.setText(habilidades?.getOrNull(0) ?: "")
            etHab2.setText(habilidades?.getOrNull(1) ?: "")
            etHab3.setText(habilidades?.getOrNull(2) ?: "")
        }
    ...
    private fun salvarPokemon() {
        ...
        if (pokemonId != null) {
            ...
            endpoint.editarPokemon(pokemon).enqueue(object : Callback<PokemonResponse> {
                override fun onResponse(call: Call<PokemonResponse>, response: Response<PokemonResponse>) {
                    if (response.isSuccessful) {
                        mostrarAlerta("Sucesso", "Pokémon atualizado com sucesso") {
                            setResult(RESULT_OK)
                            finish()
                        }
                    } else {
                        mostrarAlerta("Erro", "Erro ao atualizar Pokémon: ${response.code()}")
                    }
                }
            ...
            endpoint.cadastrarPokemon(request).enqueue(object : Callback<PokemonResponse> {
                override fun onResponse(call: Call<PokemonResponse>, response: Response<PokemonResponse>) {
                    if (response.isSuccessful) {
                        val body = response.body()
                        if (body?.sucesso == true) {
                            mostrarAlerta("Sucesso", body.mensagem ?: "Pokémon cadastrado") {
                                setResult(RESULT_OK)
                                finish()
                            }
                        } else {
                            mostrarAlerta("Erro", body?.erro ?: "Erro ao cadastrar Pokémon")
                        }
                    } else {
                        val msg = when (response.code()) {
                            409 -> "Já existe um Pokémon com esse nome"
                            400 -> "Campos obrigatórios faltando"
                            else -> "Erro ${response.code()} ao cadastrar Pokémon"
                        }
